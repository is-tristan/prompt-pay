...
export default function ComparePaymentProvidersSection({
  ...
  const handleTabClick = (tabName) => {
    ...
    const allCols = document.querySelectorAll(
      `.${styles.tabItems} [data-name]`,
    );
    allCols.forEach((col) => {
  ...
  return (
    <section className="row comparePaymentProviders">
      <div
        className={`container centered noPaddingBottom ${styles.sectionHeader} ${containerClassName ? containerClassName : undefined}`}
      >
        <Heading
          level="h2"
        ...
        <div className="container tabNav">
          {providers.map((provider) => (
            <button
              ...
              className={`tabNavItem ${provider.competitorName === activeTab ? "active" : ""}`}
            >
      ...
      <div className={`container noPaddingTop ${styles.tabItems}`}>
        {providers.map((provider) => (
          ...
          >
            <div
              className={`container centered noPaddingTop noPaddingBottom ${styles.tabItemHeader}`}
            >
              <Heading level="h3" title={provider.title} />
            </div>

            <div
              className={`${styles.comparisonContainer} noPaddingTop noPaddingBottom`}
            >
              <motion.div
                key={`benefits-${pathname}`} // Force re-animation on page changes
                ...
                {provider.benefits.length > 0 && (
                  <>
                    ...
                    {provider.benefits.map((benefit, index) => (
                      <div
                        key={index}
                        className={`${styles.listItem} ${styles.benefitItem}`}
                      >
                        <div className={styles.iconWrapper}>
                          <CheckIcon />
                        </div>

                        <p key={index} className={styles.benefitText}>
                          {benefit}
                        </p>
                      </div>
                    ))}
              ...
              <motion.div
                key={`issues-${pathname}`} // Force re-animation on page changes
              ...
              >
                {provider.issues.length > 0 && (
                  <>
                    <h4 className={styles.competitorName}>
                      {provider.competitorName}
                      <span style={{ color: "var(--primary)" }}>.</span>
                    </h4>
                    ...
                    {provider.issues.map((issue, index) => (
                      <div
                        key={index}
                        className={`${styles.listItem} ${styles.issueItem}`}
                      >
                        <div className={styles.iconWrapper}>
                          <XIcon />
                        </div>

                        <p key={index} className={styles.issueText}>
                          {issue}
                        </p>
                      </div>
                    ))}
